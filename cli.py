...
`src/api/server.py`'s existing `secrets_factory(namespace="gov",
agent_name="gov-c2-013")` call shape rather than a per-template value: one
Marketplace Pod deploys exactly one template, so there is no cross-template
secret-path collision to guard against, and `gov` is filled in at
...
if __name__ == "__main__":
    run_agent_marketplace(
        Graph,  # rename together with src/graph/graph.py's class name during design (CitizenApplicationProcessingAgent)
        agent_name="gov-c2-013",
        namespace="gov",
