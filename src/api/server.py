...
agent.provision_secrets(secrets_factory(namespace="gov", agent_name="gov-c2-013"))
...
async def invoke(req: InvokeRequest, request: Request) -> Any:
    ...
    # Standalone/STG caller auth (see the operation guide's auth section): when
    ...
    # before auth); this is the documented entry-point exception to that rule.
...
def health() -> dict[str, str]:
    return {"status": "ok", "agent": "gov-c2-013"}
